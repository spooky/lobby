...
class SiteList(object):
    def __init__(self, pointList):
        ...
        self.__xmax = 0
        self.__ymax = 0
        for uid in pointList:
            pt = pointList[uid]
            self.__sites.append(Site(pt.getx(), pt.gety(), uid))
            if pt.getx() < self.__xmin: self.__xmin = pt.getx()
            if pt.gety() < self.__ymin: self.__ymin = pt.gety()
            if pt.getx() > self.__xmax: self.__xmax = pt.getx()
            if pt.gety() > self.__ymax: self.__ymax = pt.gety()
        self.__sites.sort()
